...
<div id="content" ng-controller="blogCategory">
    <div id="popup" ng-show="popup.view">
        <div class="wrapper" ng-show="popup.view">
            ...
            <div class="popup" ng-scrollbar rebuild-on-resize rebuild-on="rebuild:scrollbar">
                <blog-category-detail ng-if="popup.blogCategory"></blog-category-detail>
            </div>
        </div>
    </div>
    ...
    <div class="wrapper" ng-init="initializePagination()">
        <div class="navigation">
            ...
            <div class="action">
                <a class="add" href="${url.base}/blog/category/entry/"><i
                        class="plus-white square-15 margin-right-5"></i>Add New</a>
            </div>
        </div>
        <div class="filter">
            <form class="filter" method="POST" action="">
                <input class="id" name="blog-category-filter-id" type="text" placeholder="ID"
                       ng-model="filter.id.value"/>
                <input name="blog-category-filter-name" type="text" placeholder="Name" ng-model="filter.name.value"/>
                <md-input-container>
                ...
                <input class="date-range-picker" name="blog-category-filter-created-timestamp" type="text"
                       placeholder="Created Date Range" ng-model="filter.createdDate.value"/>
                <button class="filter" name="blog-category-filter" ng-click="filterPagination($event)"><i
                        class="filter-white square-15 margin-right-5"></i>Filter
                </button>
                <button class="remove-filter" name="blog-category-remove-filter"
        ...
        <div class="content">
            <form class="page-entry" method="POST" action="">
                <p>Page Entry</p>
                <input name="blog-category-page-entry" type="text" placeholder="Page" ng-model="site.pagination"/>
                <button name="blog-category-page" ng-click="setPagination($event)"><i
            ...
            <table class="data-list">
                ...
                <c:forEach items="${pagination}" var="pagination" varStatus="status">
                    <tr class="${pagination._id}" ng-dblclick="loadDetail('${pagination._id}')">
                        ...
                        <td><p>${pagination.parent.name}</p></td>
                        <td><p><c:if test="${!empty pagination.thumbnail}"><img class="responsive"
                                                                                src="${url.image}/blog/category/thumbnail/${pagination.thumbnail}"/></c:if>
                        </p></td>
                        ...
                        <td class="action">
                            <div>
                                <a href="${url.base}/blog/category/entry/${pagination._id}/"><i
