...
<div id="content" ng-controller="gameTransaction">
    <div id="popup" ng-show="popup.view">
        <div class="wrapper" ng-show="popup.view">
            ...
            <div class="popup" ng-scrollbar rebuild-on-resize rebuild-on="rebuild:scrollbar">
                <game-transaction-detail ng-if="popup.gameTransaction"></game-transaction-detail>
            </div>
        </div>
    </div>
    ...
    <div class="wrapper" ng-init="initializePagination()">
        <div class="navigation">
            ...
            <div class="action">
                <c:if test="${preloodeAccount.username == 'preloode'}">
                    <a class="add" href="${url.base}/game/transaction/entry/"><i
        ...
        <div class="filter">
            <form class="filter" method="POST" action="">
                <input class="id" name="game-transaction-filter-id" type="text" placeholder="ID"
                ...
                <input class="date-range-picker" name="game-transaction-filter-created-timestamp" type="text"
                       placeholder="Created Date Range" ng-model="filter.createdDate.value"/>
                <button class="filter" name="game-transaction-filter" ng-click="filterPagination($event)"><i
                        class="filter-white square-15 margin-right-5"></i>Filter
                </button>
                <button class="remove-filter" name="game-transaction-remove-filter"
        ...
        <div class="content">
            <form class="page-entry" method="POST" action="">
                <p>Page Entry</p>
                <input name="game-transaction-page-entry" type="text" placeholder="Page" ng-model="site.pagination"/>
                <button name="game-transaction-page" ng-click="setPagination($event)"><i
            ...
            <table class="data-list">
                ...
                <c:forEach items="${pagination}" var="pagination" varStatus="status">
                    <tr class="${pagination._id}" ng-dblclick="loadDetail('${pagination._id}')">
                        ...
                        <td>
                            <c:choose>
                                <c:when test="${pagination.type == 'Commission' || pagination.type == 'Deposit' || pagination.type == 'Promotion'}">
                                    <p>${pagination.game.type.name} - ${pagination.game.name}</p>
                                </c:when>
                                <c:when test="${pagination.type == 'Top Up'}">
                                    <p>${pagination.from_bank.name} - ${pagination.from_bank.account.number}
                                        - ${pagination.from_bank.account.name}</p>
                                </c:when>
                                <c:when test="${pagination.type == 'Withdrawal'}">
                                    <p>${pagination.from_player.username}</p>
                                </c:when>
                            </c:choose>
                        </td>
                        <td>
                            <c:choose>
                                <c:when test="${pagination.type == 'Commission' || pagination.type == 'Top Up'}">
                                    <p>Provider</p>
                                </c:when>
                                <c:when test="${pagination.type == 'Deposit' || pagination.type == 'Promotion'}">
                                    <p>${pagination.to_player.username}</p>
                                </c:when>
                                <c:when test="${pagination.type == 'Withdrawal'}">
                                    <p>${pagination.game.type.name} - ${pagination.game.name}</p>
                                </c:when>
                        ...
                        <td><p>${pagination.game.type.name} - ${pagination.game.name}</p></td>
                        ...
                        <td class="action">
                            <div>
                                <c:if test="${preloodeAccount.username == 'preloode'}">
                                    <a href="${url.base}/game/transaction/entry/${pagination._id}/"><i
