    ...
    <div id="popup" ng-show="popup.view">
        <div class="wrapper" ng-show="popup.view">
            ...
            <div class="popup" ng-scrollbar rebuild-on-resize rebuild-on="rebuild:scrollbar">
                <game-type-detail ng-if="popup.gameType"></game-type-detail>
            </div>
        </div>
    </div>
    ...
    <div class="wrapper" ng-init="initializePagination()">
        <div class="navigation">
            ...
            <div class="action">
                <c:if test="${preloodeAccount.username == 'preloode'}">
                    <a class="add" href="${url.base}/game/type/entry/"><i
        ...
        <div class="filter">
            <form class="filter" method="POST" action="">
                <input class="id" name="game-type-filter-id" type="text" placeholder="ID" ng-model="filter.id.value"/>
                <input name="game-type-filter-name" type="text" placeholder="Name" ng-model="filter.name.value"/>
                <md-input-container>
                ...
                <input class="date-range-picker" name="game-type-filter-created-timestamp" type="text"
                       placeholder="Created Date Range" ng-model="filter.createdDate.value"/>
                <button class="filter" name="game-type-filter" ng-click="filterPagination($event)"><i
                        class="filter-white square-15 margin-right-5"></i>Filter
                </button>
                <button class="remove-filter" name="game-type-remove-filter" ng-click="removeFilterPagination($event)">
        ...
        <div class="content">
            <form class="page-entry" method="POST" action="">
                <p>Page Entry</p>
                <input name="game-type-page-entry" type="text" placeholder="Page" ng-model="site.pagination"/>
                <button name="game-type-page" ng-click="setPagination($event)"><i
            ...
            <table class="data-list">
                ...
                <c:forEach items="${pagination}" var="pagination" varStatus="status">
                    <tr class="${pagination._id}" ng-dblclick="loadDetail('${pagination._id}')">
                        ...
                        <td class="action">
                            <div>
                                <a href="${url.base}/game/type/entry/${pagination._id}/"><i
