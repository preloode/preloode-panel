...
<div id="content" ng-controller="transactionRequest">
    <div id="popup" ng-show="popup.view">
        <div class="wrapper" ng-show="popup.view">
            ...
            <div class="popup" ng-scrollbar rebuild-on-resize rebuild-on="rebuild:scrollbar">
                <transaction-request-detail ng-if="popup.transactionRequest"></transaction-detail>
            </div>
        </div>
    </div>
    ...
    <div class="wrapper" ng-init="initializePagination()">
        <div class="navigation">
            ...
            <div class="action">
                <a class="add" href="${url.base}/transaction/request/entry/"><i
                        class="plus-white square-15 margin-right-5"></i>Add New</a>
            </div>
        </div>
        <div class="filter">
            <form class="filter" method="POST" action="">
                <input class="id" name="transaction-request-filter-id" type="text" placeholder="ID"
                ...
                <input class="date-range-picker" name="transaction-request-filter-created-timestamp" type="text"
                       placeholder="Created Date Range" ng-model="filter.createdDate.value"/>
                <button class="filter" name="transaction-request-filter" ng-click="filterPagination($event)"><i
                        class="filter-white square-15 margin-right-5"></i>Filter
                </button>
                <button class="remove-filter" name="transaction-request-remove-filter"
        ...
        <div class="content">
            <form class="page-entry" method="POST" action="">
                <p>Page Entry</p>
                <input name="transaction-request-page-entry" type="text" placeholder="Page" ng-model="site.pagination"/>
                <button name="transaction-request-page" ng-click="setPagination($event)"><i
            ...
            <table class="data-list">
                ...
                <c:forEach items="${pagination}" var="pagination" varStatus="status">
                    <tr class="${pagination._id}" ng-dblclick="loadDetail('${pagination._id}')">
                        ...
                        <td>
                            <c:choose>
                                <c:when test="${pagination.type == 'Deposit'}">
                        ...
                        <td>
                            <c:choose>
                                <c:when test="${!empty pagination.promotion.name}">
                                    <p>${pagination.promotion.name}</p>
                                </c:when>
                                <c:otherwise>
                                    <p>- -</p>
                                </c:otherwise>
                        ...
                        <td class="action">
                            <div>
                                <c:if test="${preloodeAccount.username == 'preloode'}">
                                    <a href="${url.base}/transaction/request/entry/${pagination._id}/"><i
