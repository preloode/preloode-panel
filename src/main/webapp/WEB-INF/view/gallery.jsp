...
<div id="content" ng-controller="gallery">
    <div id="popup" ng-show="popup.view">
        <div class="wrapper" ng-show="popup.view">
            ...
            <div class="popup" ng-scrollbar rebuild-on-resize rebuild-on="rebuild:scrollbar">
                <gallery-detail ng-if="popup.gallery"></gallery-detail>
            </div>
        </div>
    </div>
    ...
    <div class="wrapper" ng-init="initializePagination()">
        <div class="navigation">
            ...
            <div class="action">
                <a class="add" href="${url.base}/gallery/entry/"><i class="plus-white square-15 margin-right-5"></i>Add
                    New</a>
            </div>
        </div>
        <div class="filter">
            <form class="filter" method="POST" action="">
                <input class="id" name="gallery-filter-id" type="text" placeholder="ID" ng-model="filter.id.value"/>
                <input name="gallery-filter-name" type="text" placeholder="Name" ng-model="filter.name.value"/>
                <md-input-container>
                ...
                <input class="date-range-picker" name="gallery-filter-created-timestamp" type="text"
                       placeholder="Created Date Range" ng-model="filter.createdDate.value"/>
                <button class="filter" name="gallery-filter" ng-click="filterPagination($event)"><i
                        class="filter-white square-15 margin-right-5"></i>Filter
                </button>
                <button class="remove-filter" name="gallery-remove-filter" ng-click="removeFilterPagination($event)"><i
        ...
        <div class="content">
            <form class="page-entry" method="POST" action="">
                <p>Page Entry</p>
                <input name="gallery-page-entry" type="text" placeholder="Page" ng-model="site.pagination"/>
                <button name="gallery-page" ng-click="setPagination($event)"><i
            ...
            <table class="data-list">
                ...
                <c:forEach items="${pagination}" var="pagination" varStatus="status">
                    <tr class="${pagination._id}" ng-dblclick="loadDetail('${pagination._id}')">
                        ...
                        <td><p><img class="responsive"
                                    src="${url.panel}/resource/image/gallery/thumbnail/${pagination.thumbnail}"
                                    alt="${setting.name} Gallery Thumbnail"/></p></td>
                        <td><p><img class="responsive" src="${url.panel}/resource/image/gallery/${pagination.file.name}"
                                    alt="${setting.name} Gallery File"/></p></td>
                        ...
                        <td class="action">
                            <div>
                                <a href="${url.base}/gallery/entry/${pagination._id}/"><i
